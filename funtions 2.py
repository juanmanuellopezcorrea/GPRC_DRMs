...
def resspoints(root,proteinpath):
    for pt in proteinpath:
        trajlst=glob.glob(os.path.join(root,pt,"coordinates","*res.csv"))#TODO quitar
        ...
        folderresrmsd = os.path.join(root,pt, "coordinates", "3Drespositions")#TODO quitar
        name="3Dres_"+pt+".csv"
        if not os.path.isdir(folderresrmsd):
            os.makedirs(folderresrmsd)
...
def UMAP_angles(datapath,prot):
    ...
    ##################################################
        ...
        embedding = reducer.fit_transform(scaled_data)
        print(embedding.shape)
        nam=os.path.join(root,"embeddings",str(traj)+'_phi_umap_allresidues.csv')
...
def UMAP_3Dposition(lstrprot):
    for prot in lstrprot[:]:
        ...
        for traj in df.trayectory.unique().tolist()[:]:
            ...
            rowsy=[]
            rowsz=[]
            print("frames",df1.framesid.unique().tolist()[:])
...
def PCA_angles_residues(datapath,prot):
    root=datapath.split("*.csv")[0]
    name="angles_"+prot+".csv"
    dfprot=pd.read_csv(os.path.join(root,"embeddings",name))
    dfprot=dfprot.sort_values(by=["idresidue",'idframe'])
    ...
    for traj in dfprot.trayectory.unique().tolist()[:]:
        print(traj)
        df1= dfprot[(dfprot.trayectory.astype(int) == int(traj))& (dfprot.idresidue.astype(int) <= int(272)) & (dfprot.idresidue.astype(int) >= int(131))]
        #dfaphi.iloc[:,3:]
        dfapsi= df1[df1.idangle.astype(str) == "apsi"]
        dfaphi= df1[df1.idangle.astype(str) == "aphi"]
        ...
        a=PCA(data,num_components=2) # YOU CAN  CHOSE THE COMPONENT NUMBER
        print("psi: ",a.shape)
        nam=os.path.join(str(traj)+'_psi_PCA_residues.csv')
        folder=os.path.join(root,"embeddings","residues","PCA")
        if not os.path.isdir(folder):
            os.makedirs(folder)
            print("saved csv:", folder)
        else:
            print("saved csv:", folder)
        np.savetxt(os.path.join(folder,nam), a, delimiter=",")
    ...
    ##################################################
        ...
        p=PCA(data,num_components=2) # YOU CAN  CHOSE THE COMPONENT NUMBER
        print("p: ",p.shape)
        nam=os.path.join(str(traj)+'_phi_PCA_residues.csv')

        np.savetxt(os.path.join(folder,nam), p, delimiter=",")
        ######################
        dfbothangle=pd.concat([dfphi,dfpsi],axis=1)
        data=dfbothangle.values
        data=np.array(data, dtype=float)
        ...
        print(phipsi.shape)
        nam=os.path.join(str(traj)+'_phi_PCA_residues.csv')
        np.savetxt(os.path.join(folder,nam), phipsi, delimiter=",")
    ...
    for traj in glob.glob(datapath)[:]:
        ...
        if not "center" in traj:
            if not "Icon"in traj:
                ...
                print("helix: ",b.shape)
                nam=os.path.join(folder,str(traj.split("/")[-1].split("_")[-2])+"_"+str("allpositions")+'_helix_PSA.csv')
                print(nam)
